...
def temp_repo_dir():
    ...
    try:
        ...
        # Explicit cleanup with Windows workaround
        import shutil
        import time
        try:
            shutil.rmtree(tmpdir)
        except PermissionError:
            # Windows file locking workaround: retry after short delay
            time.sleep(0.1)
            try:
                shutil.rmtree(tmpdir)
            except PermissionError:
                # If still fails, log and continue (temp dir will be cleaned by OS)
                pass
